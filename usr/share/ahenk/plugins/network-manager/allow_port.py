    ...
    def handle_task(self):
        try:
            ...
            for port in self.ports.split(' '):
                self.execute('iptables -D INPUT -p tcp --dport ' + port + ' -m state --state NEW,ESTABLISHED -j DROP')
                self.execute('iptables -D OUTPUT -p tcp --dport ' + port + ' -m state --state NEW,ESTABLISHED -j DROP')
            self.execute('iptables-save')

            self.logger.info('NETWORK-MANAGER - Allowing to Ports: ' + self.ports + ' are handled successfully.')
            self.context.create_response(code=self.message_code.TASK_PROCESSED.value,
                                         message='Portlar başarıyla açıldı')
