...
DEFAULT_FILE_NAME="99-block-printer.rules"

# file nane for task
FILE_NAME="${1:-$DEFAULT_FILE_NAME}"

echo 'ACTION=="add|change", SUBSYSTEM=="usb", ATTR{bInterfaceClass}=="07", ATTR{authorized}="0"
ACTION=="add|change", SUBSYSTEM=="usb", ENV{ID_USB_INTERFACES}=="*:07:*", ATTR{authorized}="0"
